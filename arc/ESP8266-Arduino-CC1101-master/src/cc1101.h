...
#define CC1101_SFSTXON                                                         \
  0x31 // Enable and calibrate frequency synthesizer (if MCSM0.FS_AUTOCAL=1). If
       // in RX (with CCA): Go to a wait state where only the synthesizer is
       // running (for quick RX / TX turnaround).
#define CC1101_SXOFF 0x32 // Turn off crystal oscillator
#define CC1101_SCAL                                                            \
  0x33 // Calibrate frequency synthesizer and turn it off. SCAL can be strobed
...
#define CC1101_STX                                                             \
  0x35 // In IDLE state: Enable TX. Perform calibration first if
       // MCSM0.FS_AUTOCAL=1. If in RX state and CCA is enabled: Only go to TX
       // if channel is clear
#define CC1101_SIDLE                                                           \
...
#define CC1101_SFRX                                                            \
  0x3A // Flush the RX FIFO buffer. Only issue SFRX in IDLE or RXFIFO_OVERFLOW
       // states
#define CC1101_SFTX                                                            \
  0x3B // Flush the TX FIFO buffer. Only issue SFTX in IDLE or TXFIFO_UNDERFLOW
       // states
 ...
 * Length config = Variable packet length mode. Packet length configured by the
 * first byte after sync word CRC enable = true Packet length = 255 Device
 * address = 1 Address config = Enable address check Append status = Append two
 * status bytes to the payload of the packet. The status bytes contain RSSI and
 * LQI values, as well as CRC OK
 * CRC autoflush = false
 * PA ramping = false
 * TX power = 12
 * GDO0 mode = Asserts when sync word has been sent / received, and de-asserts
 * at the end of the packet. In RX, the pin will also de-assert when a packet is
 * discarded due to address or maximum length filtering or when the radio enters
...
#define CC1101_DEFVAL_MCSM2                                                    \
  0x07 // Main Radio Control State Machine Configuration
//#define CC1101_DEFVAL_MCSM1      0x30        // Main Radio Control State
//Machine Configuration
#define CC1101_DEFVAL_MCSM1                                                    \
  0x20 // Main Radio Control State Machine Configuration
#define CC1101_DEFVAL_MCSM0                                                    \
...
public:
   ...
   *
   * 'regAddr'	Register address
   * 'regType'	Type of register: CC1101_CONFIG_REGISTER or
