...
bool eSPIClass::pins(int8_t sck, int8_t miso, int8_t mosi, int8_t ss)
{
    if (sck == 6 &&
        miso == 7 &&
        mosi == 8 &&
...
void ICACHE_RAM_ATTR eSPIClass::end() {
    switch (pinSet) {
    case SPI_PINS_HSPI:
        pinMode(SCK, INPUT);
        pinMode(MISO, INPUT);
...
void ICACHE_RAM_ATTR eSPIClass::setDataMode(uint8_t dataMode) {
...
inline void ICACHE_RAM_ATTR eSPIClass::setDataBits(uint16_t bits) {
    const uint32_t mask = ~((SPIMMOSI << SPILMOSI) | (SPIMMISO << SPILMISO));
    bits--;
    SPI1U1 = ((SPI1U1 & mask) | ((bits << SPILMOSI) | (bits << SPILMISO)));
}

uint8_t ICACHE_RAM_ATTR eSPIClass::transfer(uint8_t data) {
    while(SPI1CMD & SPIBUSY) {}
    // reset to 8Bit mode
    setDataBits(8);
    SPI1W0 = data;
...
void ICACHE_RAM_ATTR eSPIClass::write(uint8_t data) {
    while(SPI1CMD & SPIBUSY) {}
    // reset to 8Bit mode
    setDataBits(8);
    SPI1W0 = data;
...
void ICACHE_RAM_ATTR eSPIClass::write16(uint16_t data) {
    write16(data, !(SPI1C & (SPICWBO | SPICRBO)));
}

void ICACHE_RAM_ATTR eSPIClass::write16(uint16_t data, bool msb) {
    while(SPI1CMD & SPIBUSY) {}
...
void ICACHE_RAM_ATTR eSPIClass::write32(uint32_t data) {
    write32(data, !(SPI1C & (SPICWBO | SPICRBO)));
}

void ICACHE_RAM_ATTR eSPIClass::write32(uint32_t data, bool msb) {
    while(SPI1CMD & SPIBUSY) {}
 ...
 * @param size uint32_t
 */
void ICACHE_RAM_ATTR eSPIClass::writeBytes(uint8_t * data, uint32_t size) {
    while(size) {
        if(size > 64) {
            writeBytes_(data, 64);
            size -= 64;
...
void ICACHE_RAM_ATTR eSPIClass::writeBytes_(uint8_t * data, uint8_t size) {
    while(SPI1CMD & SPIBUSY) {}
 ...
 */
void ICACHE_RAM_ATTR eSPIClass::writePattern(uint8_t * data, uint8_t size, uint32_t repeat) {
 ...
 * @param size uint32_t
 */
void ICACHE_RAM_ATTR eSPIClass::transferBytes(uint8_t * out, uint8_t * in, uint32_t size) {
    while(size) {
        if(size > 64) {
...
void ICACHE_RAM_ATTR eSPIClass::transferBytes_(uint8_t * out, uint8_t * in, uint8_t size) {
    while(SPI1CMD & SPIBUSY) {}
